...
public class ChatActivity extends AppCompatActivity {
    ...
    private SimpleDateFormat simpleDateFormat = new SimpleDateFormat("aa hh:mm");
    ...
    Button.OnClickListener sendBtnClickListener = new View.OnClickListener() {
        public void onClick(View view) {
            ...
            if (roomID==null) {             // two user
                ...
                FirebaseDatabase.getInstance().getReference().child("rooms/"+roomID).setValue(chatModel);
                recyclerView.setLayoutManager(new LinearLayoutManager(ChatActivity.this));
                recyclerView.setAdapter(new RecyclerViewAdapter());
            }
            ...
            FirebaseDatabase.getInstance().getReference().child("rooms").child(roomID).child("messages").push().setValue(messages).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
    ...
    class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
        ...
        public RecyclerViewAdapter() {
            ...
            databaseReference = FirebaseDatabase.getInstance().getReference().child("rooms").child(roomID).child("messages");
            valueEventListener = databaseReference.addValueEventListener(new ValueEventListener(){
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    for (DataSnapshot item: dataSnapshot.getChildren()) {
                        ChatModel.Message message_origin = item.getValue(ChatModel.Message.class);
                        ChatModel.Message message_modify = item.getValue(ChatModel.Message.class);
                        message_modify.readUsers.put(myUid, true);
                        readUsers.put(item.getKey(), message_modify);
                        messageList.add(message_origin);
                    }

                    if (messageList.size()>0 && !messageList.get(messageList.size()-1).readUsers.containsKey(myUid)) {
                        FirebaseDatabase.getInstance().getReference().child("rooms").child(roomID).child("messages").updateChildren(readUsers).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            ...
            String timestamp = simpleDateFormat.format( new Date( (long) message.timestamp) );
